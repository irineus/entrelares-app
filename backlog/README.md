...
| **T-63** | **Retire the backlog mirror** — the Notion card becomes the single record of an item, the way the Gestão IM360 and Desmalha boards already work (owner, 07/09/2026: *"replicação desnecessária"*). One-time migration of every pending record into its card body (the mirror's own LAST run), `next-item` rewritten to read/write the card like `proxima-tarefa`, `tool/notion_mirror.py` deleted, `backlog/` frozen as history. Gap questions (trailer, `Esforço gasto`, landing, `Fase`) in the record. **The last item delivered under the mirror scheme.** |
